function getSelected(event) {
    ...
    if (
        color[0] >= 0 &&
        color[0] <= boardLength - 1 &&
        color[1] >= 0 &&
        color[1] <= boardLength - 1 &&
        (color[2] == 38 || color[2] == 171 || color[2] == 178)
    )
...
function drawHexagon(ctx, x, y, radius) {
    ...
    for (var i = 0; i < 6; i++)
        ctx.lineTo(
            x + radius * Math.cos(Math.PI * (1.5 + (1 / 3) * i)),
            y + radius * Math.sin(Math.PI * (1.5 + (1 / 3) * i))
        );
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
...
function drawPath(ctx, p) {
    ...
    ctx.beginPath();
    ctx.moveTo(
        (p[0][0] + p[0][1]) * width - (p[0][1] - 4) * (width / 2),
        (p[0][1] + 2) * 1.5 * radius
    );
    for (var i = 1; i < p.length; i++)
        ctx.lineTo(
            (p[i][0] + p[i][1]) * width - (p[i][1] - 4) * (width / 2),
            (p[i][1] + 2) * 1.5 * radius
        );
    ctx.stroke();
}
...
function draw(board, player, selected, width, radius, ctx) {
    ...
    const [right, bottom] = getDrawCoordinates(
        boardLength - 1,
        boardLength - 1,
        width,
        radius
